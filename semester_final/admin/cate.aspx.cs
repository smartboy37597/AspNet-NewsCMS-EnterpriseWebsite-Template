...
namespace semester_final.admin
{
    public partial class cate : System.Web.UI.Page
    {
        ...
        protected void programObj_ItemCommand(object source, RepeaterCommandEventArgs e)
        {
            ...
            {
                case "Delete":
                    string sql = "DELETE FROM fp_product_column WHERE id = '" + pId + "'";
                    MyLib.sqlQueryOld(sql);
                    Response.Write("<script>alert('删除产品栏目成功');location.href='cate.aspx'</script>");
                    break;
        ...
        protected void addPColumnBtn_Click(object sender, EventArgs e)
        {
            ...
            string sql = "INSERT INTO fp_product_column (name) VALUES ('" + title + "')";
            MyLib.sqlQueryOld(sql);
            Response.Write("<script>alert('新建产品栏目成功');location.href='cate.aspx'</script>");
